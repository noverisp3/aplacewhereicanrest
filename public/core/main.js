...
function applyTheme(){var t=localStorage.getItem('theme')||'light';document.documentElement.className=t;setTimeout(function(){document.documentElement.classList.add('theme-ready')},100);var b=document.getElementById('theme-toggle');if(b)b.textContent=t.charAt(0);document.querySelectorAll('.theme-option').forEach(function(o){o.classList.toggle('active',o.dataset.theme===t)})}
  ...
  fetch(API + '/api/activity', { headers: authHeaders() })
    ...
    .then(function(data) {
      ...
      for (var i = 44; i >= 0; i--) {
        ...
        var c = map[key] || 0;
        var lvl = c === 0 ? '' : c < 5 ? 'l1' : c < 10 ? 'l2' : c < 15 ? 'l3' : 'l4';
